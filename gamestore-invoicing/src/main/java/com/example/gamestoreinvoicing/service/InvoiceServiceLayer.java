...
public class InvoiceServiceLayer {
    ...
    @Autowired
    public InvoiceServiceLayer(InvoiceRepository invoiceRepo, TaxRepository taxRepo, ProcessingFeeRepository processingFeeRepo, GameStoreCatalog client) {
...
//    GameStoreCatalog client;
    ...
    public InvoiceViewModel createInvoice(InvoiceViewModel invoiceViewModel) {
        ...
        if (invoiceViewModel.getItemType().equals(CONSOLE_ITEM_TYPE)) {
            ...
            if (invoiceViewModel.getQuantity() <= returnVal.getQuantity()){
                long updatedQuant =  returnVal.getQuantity() - invoiceViewModel.getQuantity();

                returnVal.setQuantity(updatedQuant);
                client.updateConsoleQuant(returnVal);
            }
            ...
            if (invoiceViewModel.getQuantity() <= returnVal.getQuantity()){
                ...
                returnVal.setQuantity(updatedQuant);
                client.updateGameQuant(returnVal);
            ...
            if (invoiceViewModel.getQuantity() <= returnVal.getQuantity()) {
                ...
                returnVal.setQuantity(updatedQuant);
                client.updateTshirtQuant(returnVal);
            }
            invoice.setUnitPrice(returnVal.getPrice());
